...
import com.email.utils.TranCharsetUtil;
...
public class MailReceiver implements Serializable {
			    ...
			    if (part.getContent() instanceof Multipart) {
			    	...
			    	try {
						InputStream is = part.getInputStream();
							...
							Log.e("===length====", is.available()+"");
							if (is.available()>10*1024*1024) {
								mailContent.append("内容过大，无法加载！");
							}else{
						ByteArrayDataSource bads = new ByteArrayDataSource(is, "multipart/*");
						Multipart multipart = new MimeMultipart(bads);
						int counts = multipart.getCount();
						for (int i = 0; i < counts; i++) {
						    compileMailContent(multipart.getBodyPart(i));
						}
    ...
    private String parseCharset(String contentType) {
        ...
        if (contentType.contains("gbk")) {
            ...
            return "gb2312";
        }else if (contentType.contains("UTF8") || contentType.contains("UTF-8")) {
            return "UTF-8";
        } else {
