...
public class HttpUtil {
	...
	private Session isLoginRight(MailInfo info) {
		...
		if(info.isValidate()){
			//创建一个密码验证器
			Log.e("=====n==p====", info.getUserName()+"--------------"+ info.getPassword());
...
//			String username =name.replaceAll("_", "%5F");
//			Log.e("=====name=====", username+"============"+info.getPassword());
			Log.e("=======host====", info.getMailServerHost()+"---"+info.getMailServerPort()+"----"+info.getUserName()+"----"+info.getPassword());
	...
	public boolean sendTextMail(MailInfo mailInfo, Session sendMailSession) {
		...
		try {
			...
			mdp.setContent(mailContent, "text/html;charset=utf8");// 给BodyPart对象设置内容和格式/编码方式
